...
from config import BASKET_WARNING_ZONE_REGION_TOP,BASKET_WARNING_ZONE_REGION_FORNT,BASKET_EMPTY_LOAD_REGION
...
def save_image(basket_cleaning_imgs,basket_cleaning_order, results, step_name,basket_seg_region):
    ...
    if step_name == "basket_step_1":
        annotated_frame = cv2.polylines(annotated_frame, [BASKET_WARNING_ZONE_REGION_TOP.reshape(-1, 1, 2)], isClosed=True, color=(0, 255, 0), thickness=4)
...
def fetch_video_stream(rtsp_url, frame_queue_list, start_event, stop_event):  # 拉取视频流到队列中
    #队列与对应的模型
    #frame_queue_list[0]:吊篮正面视角，检测警戒区
    #frame_queue_list[1]:吊篮悬挂机构视角，检测人是否在检查悬挂机构，用姿态估计
    #frame_queue_list[2]:吊篮顶部视角，检测安全带,刷子，警戒区，自锁器
    #frame_queue_list[3]:吊篮顶部视角，进行分割
    #frame_queue_list[4]:吊篮顶部视角，检测人是否检查对应的区域
...
def infer_yolo(model_path,video_source, start_event, stop_event,basket_cleaning_flag,basket_cleaning_imgs,basket_cleaning_warning_zone_flag,basket_cleaning_order,basket_seg_region):#YOLO模型推理
    ...
    while True:      
        ...
        if model_path==WEIGHTS_BASKET[0]:#D5,detect
        ...
        elif model_path==WEIGHTS_BASKET[1] and not basket_cleaning_flag[1]:#D4悬挂机构
        ...
        elif model_path==WEIGHTS_BASKET[2]:#d6目标检测
            boxes = results[0].boxes.xyxy  
            confidences = results[0].boxes.conf 
            classes = results[0].boxes.cls  
            ...
            for i in range(len(boxes)):
                ...
                cls = int(classes[i].item())
                label = model.names[cls]
                if label=="basket" and confidence>0.85:
