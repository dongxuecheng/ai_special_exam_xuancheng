    ...
    while True:      
        ...
        if model_path==WEIGHTS_WELDING_WEARING[1]:
            ...
            if welding_wearing_human_in_postion.value and not welding_wearing_detection_img_flag.value:
                welding_wearing_items_nums[0] = max(welding_wearing_items_nums[2], wearing_items["helmet"])
                welding_wearing_items_nums[1] = max(welding_wearing_items_nums[3], wearing_items["gloves"])
                welding_wearing_items_nums[2] = max(welding_wearing_items_nums[4], wearing_items["shoes"])
